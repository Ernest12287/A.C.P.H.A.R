// src/commands/nsfw/oral.js

export default {
    name: 'oral',
    aliases: ['blowjob', 'bj'],
    description: 'Sends a random NSFW oral image. (Premium, Private Only)',
    isPremium: true,
    ...
    async execute(sock, message, args, logger, { isPremiumUser, isGroup }) {
        ...
        try {
            // Fetch a random NSFW oral image from Waifu.im
            const response = await fetch('https://api.waifu.im/search?included_tags=oral'); // Oral is an NSFW tag
            const data = await response.json();
            const imageUrl = data.images[0]?.url;

            if (!imageUrl) {
                logger.warn(`Waifu.im Oral API returned no image URL for ${chatId}`);
                return await sock.sendMessage(chatId, { text: '❌ *ACEPHAR NSFW Error*\n\nCould not fetch an oral image. Please try again later.' }, { quoted: message });
            }

            await sock.sendMessage(chatId, {
                image: { url: imageUrl },
                caption: '🔞 Your exclusive ACEPHAR oral art! 🔞\n\n_Powered by Ernest Tech House_'
            }, { quoted: message });
            logger.info(`Sent oral image to ${chatId}`);

        } catch (error) {
            logger.error(`Error in oral command for ${chatId}:`, error);
            await sock.sendMessage(chatId, { text: '❌ *ACEPHAR System Error*\n\nFailed to fetch oral image due to an internal error.' }, { quoted: message });
        }
    },
};