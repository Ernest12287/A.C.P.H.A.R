// src/commands/nsfw/paizuri.js

export default {
    name: 'paizuri',
    aliases: [],
    description: 'Sends a random NSFW paizuri image. (Premium, Private Only)',
    isPremium: true,
    ...
    async execute(sock, message, args, logger, { isPremiumUser, isGroup }) {
        ...
        try {
            const response = await fetch('https://api.waifu.im/search?included_tags=paizuri'); // Paizuri is an NSFW tag
            const data = await response.json();
            const imageUrl = data.images[0]?.url;

            if (!imageUrl) {
                logger.warn(`Waifu.im Paizuri API returned no image URL for ${chatId}`);
                return await sock.sendMessage(chatId, { text: '❌ *ACEPHAR NSFW Error*\n\nCould not fetch a paizuri image. Please try again later.' }, { quoted: message });
            }

            await sock.sendMessage(chatId, {
                image: { url: imageUrl },
                caption: '🍈 Your exclusive ACEPHAR paizuri! 🍈\n\n_Powered by Ernest Tech House_'
            }, { quoted: message });
            logger.info(`Sent paizuri image to ${chatId}`);

        } catch (error) {
            logger.error(`Error in paizuri command for ${chatId}:`, error);
            await sock.sendMessage(chatId, { text: '❌ *ACEPHAR System Error*\n\nFailed to fetch paizuri image due to an internal error.' }, { quoted: message });
        }
    },
};