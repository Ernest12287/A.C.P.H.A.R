...
export default {
    name: 'ass',
    aliases: ['butt'],
    description: 'Sends a random NSFW ass image. (Premium, Private Only)',
    isPremium: true,
    ...
    async execute(sock, message, args, logger, { isPremiumUser, isGroup }) {
        ...
        try {
            const response = await fetch('https://api.waifu.im/search?included_tags=ass'); // Ass is an NSFW tag
            const data = await response.json();
            const imageUrl = data.images[0]?.url;

            if (!imageUrl) {
                logger.warn(`Waifu.im Ass API returned no image URL for ${chatId}`);
                return await sock.sendMessage(chatId, { text: '❌ *ACEPHAR NSFW Error*\n\nCould not fetch an ass image. Please try again later.' }, { quoted: message });
            }

            await sock.sendMessage(chatId, {
                image: { url: imageUrl },
                caption: '🍑 Your exclusive ACEPHAR ass art! 🍑\n\n_Powered by Ernest Tech House_'
            }, { quoted: message });
            logger.info(`Sent ass image to ${chatId}`);

        } catch (error) {
            logger.error(`Error in ass command for ${chatId}:`, error);
            await sock.sendMessage(chatId, { text: '❌ *ACEPHAR System Error*\n\nFailed to fetch ass image due to an internal error.' }, { quoted: message });
        }
    },
};