// src/commands/nsfw/raiden-shogun.js

export default {
    name: 'raidenshogun',
    aliases: ['raiden'],
    description: 'Sends a random NSFW Raiden Shogun image. (Premium, Private Only)',
    isPremium: true,
    ...
    /**
     * Executes the raiden-shogun command.
    ...
    async execute(sock, message, args, logger, { isPremiumUser, isGroup }) {
        ...
        try {
            const response = await fetch('https://api.waifu.im/search?included_tags=raiden-shogun&is_nsfw=true');
            const data = await response.json();
            const imageUrl = data.images[0]?.url;

            if (!imageUrl) {
                logger.warn(`Waifu.im Raiden Shogun API returned no image URL for ${chatId}`);
                return await sock.sendMessage(chatId, { text: '❌ *ACEPHAR NSFW Error*\n\nCould not fetch a Raiden Shogun image. Please try again later.' }, { quoted: message });
            }

            await sock.sendMessage(chatId, {
                image: { url: imageUrl },
                caption: '⚡ Your exclusive ACEPHAR Raiden Shogun! ⚡\n\n_Powered by Ernest Tech House_'
            }, { quoted: message });
            logger.info(`Sent Raiden Shogun image to ${chatId}`);

        } catch (error) {
            logger.error(`Error in raiden-shogun command for ${chatId}:`, error);
            await sock.sendMessage(chatId, { text: '❌ *ACEPHAR System Error*\n\nFailed to fetch Raiden Shogun image due to an internal error.' }, { quoted: message });
        }
    },
};