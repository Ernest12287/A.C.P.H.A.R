// src/commands/nsfw/uniform.js

export default {
    name: 'uniform',
    aliases: [],
    description: 'Sends a random NSFW uniform image. (Free, Private Only)',
    isPremium: false,
    ...
    async execute(sock, message, args, logger, context) {
        ...
        try {
            ...
            if (!imageUrl) {
                logger.warn(`Waifu.im Uniform API returned no image URL for ${chatId}`);
                return await sock.sendMessage(chatId, { text: '❌ *ACEPHAR NSFW Error*\n\nCould not fetch a uniform image. Please try again later.' }, { quoted: message });
            }

            await sock.sendMessage(chatId, {
                image: { url: imageUrl },
                caption: '🎓 Your ACEPHAR uniform! 🎓\n\n_Powered by Ernest Tech House_'
            }, { quoted: message });
            logger.info(`Sent uniform image to ${chatId}`);

        } catch (error) {
            logger.error(`Error in uniform command for ${chatId}:`, error);
            await sock.sendMessage(chatId, { text: '❌ *ACEPHAR System Error*\n\nFailed to fetch uniform image due to an internal error.' }, { quoted: message });
        }
    },
};
