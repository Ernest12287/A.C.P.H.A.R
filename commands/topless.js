// src/commands/nsfw/topless.js

export default {
    name: 'topless',
    aliases: [],
    description: 'Sends a random NSFW topless image. (Premium, Private Only)',
    isPremium: true,
    ...
    async execute(sock, message, args, logger, { isPremiumUser, isGroup }) {
        ...
        try {
            // Using 'oppai' tag from Waifu.im which often implies topless content in NSFW context
            const response = await fetch('https://api.waifu.im/search?included_tags=oppai&is_nsfw=true');
            const data = await response.json();
            const imageUrl = data.images[0]?.url;

            if (!imageUrl) {
                logger.warn(`Waifu.im Topless API returned no image URL for ${chatId}`);
                return await sock.sendMessage(chatId, { text: '❌ *ACEPHAR NSFW Error*\n\nCould not fetch a topless image. Please try again later.' }, { quoted: message });
            }

            await sock.sendMessage(chatId, {
                image: { url: imageUrl },
                caption: '👙 Your exclusive ACEPHAR topless! 👙\n\n_Powered by Ernest Tech House_'
            }, { quoted: message });
            logger.info(`Sent topless image to ${chatId}`);

        } catch (error) {
            logger.error(`Error in topless command for ${chatId}:`, error);
            await sock.sendMessage(chatId, { text: '❌ *ACEPHAR System Error*\n\nFailed to fetch topless image due to an internal error.' }, { quoted: message });
        }
    },
};