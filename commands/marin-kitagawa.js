// src/commands/nsfw/marin-kitagawa.js

export default {
    name: 'marinkitagawa',
    aliases: ['marin'],
    description: 'Sends a random NSFW Marin Kitagawa image. (Premium, Private Only)',
    isPremium: true,
    ...
    /**
     * Executes the marin-kitagawa command.
    ...
    async execute(sock, message, args, logger, { isPremiumUser, isGroup }) {
        ...
        try {
            const response = await fetch('https://api.waifu.im/search?included_tags=marin-kitagawa&is_nsfw=true');
            const data = await response.json();
            const imageUrl = data.images[0]?.url;

            if (!imageUrl) {
                logger.warn(`Waifu.im Marin Kitagawa API returned no image URL for ${chatId}`);
                return await sock.sendMessage(chatId, { text: '❌ *ACEPHAR NSFW Error*\n\nCould not fetch a Marin Kitagawa image. Please try again later.' }, { quoted: message });
            }

            await sock.sendMessage(chatId, {
                image: { url: imageUrl },
                caption: '💖 Your exclusive ACEPHAR Marin Kitagawa! 💖\n\n_Powered by Ernest Tech House_'
            }, { quoted: message });
            logger.info(`Sent Marin Kitagawa image to ${chatId}`);

        } catch (error) {
            logger.error(`Error in marin-kitagawa command for ${chatId}:`, error);
            await sock.sendMessage(chatId, { text: '❌ *ACEPHAR System Error*\n\nFailed to fetch Marin Kitagawa image due to an internal error.' }, { quoted: message });
        }
    },
};